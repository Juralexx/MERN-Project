...
import { Button, IconToggle } from '../tools/components/Button';
...
const NewConversationModal = ({ user, friends, currentId, changeCurrentChat, websocket }) => {
    ...
    useEffect(() => {
        if (open && user.friends.length > 0) {
            const getFriends = async () => {
                const friendsArr = friends.map(async element => {
                    return await axios
                        .get(`${process.env.REACT_APP_API_URL}api/user/${element.friend}`)
                        .then(res => res.data)
                        .catch(err => console.error(err))
                })
                Promise.all(friendsArr).then(res => setFriendsFound(res))
            }
            getFriends()
        }
    }, [user.friends, open])
    ...
    const createNewConversation = async () => {
        ...
        let member = { id: user._id, pseudo: user.pseudo, picture: user.picture }
        await axios({
            method: "post",
    ...
    return (
        <>
            <IconToggle icon={<AiOutlinePlusCircle />} onClick={() => setOpen(true)} />
