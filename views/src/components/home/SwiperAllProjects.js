...
const SwiperAllProjects = ({ projects }) => {
    const [isLoading, setLoading] = useState(false)
    const [sortedTable, setSortedtable] = useState([])
    const [openModal, setOpenModal] = useState(false)

    useEffect(() => { setSortedtable(projects) }, [projects])
    ...
    return (
        ...
        >
            {sortedTable.map((element, key) => {
                return (
                    <div key={key}>
                        {openModal && <ProjectModal project={element} openModal={openModal} setOpenModal={setOpenModal} />}
                        <SwiperSlide
                            key={key}
                            style={{ width: "33.333%", display: "flex", justifyContent: "center" }}>

                            {isLoading && (<Oval />)}
                            <div className="home-project-card">
                                <div className="left" style={projectPicture('img/paysage-2.jpg')}>
                                    <MdZoomOutMap className="deploy-project" onClick={() => setOpenModal(!openModal)} />
...
export default SwiperAllProjects;