...
const AddMember = ({ open, setOpen, project, user }) => {
    ...
    const addMembers = async () => {
        if (array.length > 0) {
            await axios({
                method: "put",
                url: `${process.env.REACT_APP_API_URL}api/project/add-user/${project._id}`,
                data: {
                    members: array,
                }
            }).then((res) => {
        ...
        <Modal open={open} setOpen={setOpen} css="bg-white dark:bg-background_primary shadow-custom dark:shadow-lg">
            ...
            {displayMembers && (
                <div className="relative min-h-[400px]">
                            ...
                            <>
                                ...
                                <div className="h-full overflow-auto">
                                    {friendsFound.map((element, key) => {
                                        return (
                                            <div className="items-center p-2 my-1 cursor-pointer rounded-lg"
                                                key={key}
                    ...
                    <Button text="Ajouter" className="mt-5" disabled={array.length === 0} onClick={addMembers} />
                </div>
            )}
        </Modal>
    )
